...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.sim_weather = 'random'  # options: 'random', '_fog_0.060', '_snow_2.5_2.0'
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            try:
                ...
                # Skip sample if missing
                print(f"[WARN] Missing lidar data for timestamp {timestamp_key_delay}, skipping sample.")
            ...
            if self.train_sim and self.train:
                ...
                if  aug <= 2 :
                    ...
                    if self.sim_weather == 'random':
                        
                        weather_options = ['_fog_0.060', '', '_snow_2.5_2.0']
                        chosen_weather = random.choice(weather_options)
                        lidar_file = lidar_file.replace('.pcd', chosen_weather + '.pcd')
                    else:
                        lidar_file = lidar_file.replace('.pcd', self.sim_weather + '.pcd')
    ...
    def calc_dist_to_ego(self, scenario_database, timestamp_key):
        ...
        for cav_id, cav_content in scenario_database.items():
            if cav_content['ego']:
                ...
                try:
                    ...
                    print(f"KeyError: Missing timestamp {timestamp_key} in scenario. Available keys: {list(cav_content.keys())}")
                    # Skip this timestamp or use fallback
                    return None  # This will cause the sample to be skipped
                break
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            try:
                ...
                print(f"KeyError: Missing timestamp {timestamp_key} for CAV {cav_id}")
                continue  # Skip this CAV
            distance = \
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        try:
            ...
            return {
                ...
                'gt_transformation_matrix': np.eye(4),
                'spatial_correction_matrix': np.eye(4),
                'l2c_transformation_matrix': np.eye(4),
                'c2e_transformation_matrix': np.eye(4)
            }
        ...
        required_keys = ['lidar_pose']
        for params in [cur_params, delay_params]:
            for key in required_keys:
                if key not in params:
                    params[key] = [0, 0, 0, 0, 0, 0]  # default pose
                    print(f"Warning: Missing {key} in YAML, using default")
        ...
        # Add safety checks for camera parameters
        if 'camera0' not in cur_params:
            print(f"Warning: 'camera0' missing in cur_params")
            cur_params['camera0'] = {'cords': [0, 0, 0, 0, 0, 0]}

        print(f"DEBUG: Available keys in cur_params: {list(cur_params.keys())}")
        print(f"DEBUG: Available keys in delay_params: {list(delay_params.keys())}")
        # we always use current timestamp's gt bbx to gain a fair evaluation
        #delay_params['vehicles'] = cur_params['vehicles']
        # Safely copy vehicles data if it exists
        if 'vehicles' in cur_params:
            delay_params['vehicles'] = cur_params['vehicles']
        else:
            # Create empty vehicles dict if missing
            delay_params['vehicles'] = {}
            print(f"Warning: 'vehicles' key missing in YAML file")
        # Safety check for transformation matrices (ensure they're valid 4x4)
        matrices_to_check = [
            ('transformation_matrix', transformation_matrix),
            ('gt_transformation_matrix', gt_transformation_matrix), 
            ('spatial_correction_matrix', spatial_correction_matrix),
            ('l2c_transformation_matrix', l2c_transformation_matrix),
            ('c2e_transformation_matrix', c2e_transformation_matrix)
        ]

        for matrix_name, matrix in matrices_to_check:
            if matrix.shape != (4, 4):
                print(f"Warning: {matrix_name} has wrong shape {matrix.shape}, using identity")
                matrix = np.eye(4)
            delay_params[matrix_name] = matrix
